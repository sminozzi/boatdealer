...
class Tab {
	...
	public function __construct ( $title, $id, $page, $section_option_settings = array(), $active_state = FALSE ) 
	{
		...
		$this->id = $id;
 	    ...
 	    $this->anchor = '<a href="?page=' . $this->page->slug . '&post_type=boats&tab=' . $this->id . '" class="nav-tab">' . $this->title . '</a>';
	...
	public function get_anchor( $active = false ) 
	{
		...
		if($active) {
			return '<a href="?page=' . $this->page->slug . '&post_type=boats&tab=' . $this->id . '" class="nav-tab nav-tab-active">' . $this->title . '</a>';
		}
		return $this->anchor;
	}
